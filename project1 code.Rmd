...
## --- Q5 (Yield part): covariance of daily log-returns of 1..5y yields ---

m_grid <- 1:5                      # maturities i = 1,...,5
D <- sort(unique(ytm_panel$Date))  # the 10 dates (should match all_dates)

# 1) build a 10x5 matrix R where R[j,i] = r_{i,j} = yield at maturity i on date j
...
# 2) daily log-returns: X_{i,j} = log(r_{i,j+1}/r_{i,j})
X <- log(R[-1, , drop = FALSE] / R[-nrow(R), , drop = FALSE])
colnames(X) <- paste0("X", m_grid)

# 3) covariance matrix of (X1,...,X5)
...
D <- sort(unique(spot_panel$Date))   # dates (should be 10)
t_grid <- 1:5                        # need s(1),...,s(5)
t1 <- 1
t2_vec <- 2:5                        # gives f(1,2), f(1,3), f(1,4), f(1,5)

# Forward rate matrix: rows = dates, cols = forward tenors
...
# Daily log-returns X_{i,j} = log(r_{i,j+1} / r_{i,j})
